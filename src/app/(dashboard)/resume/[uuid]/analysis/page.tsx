...
export default function ResumeAnalysisPage() {
  ...
  const { query, mutation } = useResumeAnalysis(uuid);

  // UPDATED: detect cached response and inform user
const handleAnalyze = async () => {
  try {
    const result = await mutation.mutateAsync(uuid) as ResumeAnalysis & { cached?: boolean };
    if (result.cached) {
      toast.info("Your resume hasn't changed since the last analysis. Edit your resume first to get fresh results.", {
        duration: 6000,
      });
    }
  } catch (err) {
  ...
  return (
    <PageTransition>
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-text-primary">ATS Analysis</h1>
        ...
        {noAnalysis && (
          <EmptyState
            ...
            description="Run an ATS analysis to get insights on your resume's performance."
            action={{
              label: "Run Analysis",
